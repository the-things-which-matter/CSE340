...
Util.buildClassificationGrid = async function(data) {
  ...
  if (data.length > 0) {
    ...
    data.forEach(vehicle => { 
      // Log the image path and full URL we're building
      console.log('Image filename:', vehicle.inv_thumbnail);
      console.log('Full image URL:', '/images/' + vehicle.inv_thumbnail);

      grid += '<li>';
      grid += '<a href="/inv/detail/' + vehicle.inv_id 
           + '" title="View ' + vehicle.inv_make + ' ' + vehicle.inv_model + ' details">';
      grid += '<img src="/images/' + vehicle.inv_thumbnail 
  ...
  if (data.length > 0) {
    ...
    data.forEach(vehicle => { 
      ...
      if (imageFilename.startsWith('vehicles/')) {
        // Do nothing, already has vehicles/ prefix
      } else {
      ...
      console.log('Processed full image URL:', fullImageUrl);
